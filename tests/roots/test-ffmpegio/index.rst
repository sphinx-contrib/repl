...
All the media I/O operations in `ffmpegio` support FFmpeg filtering via per-stream 
...
The `ffmpegio.filtergraph` submodule is designed to assist building complex filtergraphs. The 
...
All `ffmpegio` functions that take filter options accept these objects as input arguments
